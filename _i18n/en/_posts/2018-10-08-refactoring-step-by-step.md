---
layout: post
title: Refactoring step by step - part 1
...
It was long break from last technical article. During that time I was trying many different things. You can look here: <a href="https://bemore.womanonrails.com/" title="Be more - Life style blog" target="_blank">Be more - my lifestyle blog in Polish</a>, <a href="https://www.youtube.com/channel/UCudKRFuddrf8saaxUEoo0xQ" title="Woman on Rails YouTube channel" target="_blank" rel="nofollow noopener noreferrer">Woman on Rails YouTube channel</a> and my <a href="https://vimeo.com/womanonrails" title="Woman on Rails - Vimeo channel" target="_blank" rel="nonofollow noopener noreferrer">travel Vimeo channel</a>. It was great time to discover what I like to do and what not. But back to topic. For this article I prepared for a long time. I can say even too long. I started in 2015 and now you will see the results. Let's get started.

Refactoring is one of my favorite topics. I love to clean up things in real life and also in code. I worked and I'm still working on web application projects. And I look for answers how to write good code. What are the reasons why after some time our code is messy and not readable? So day by day I'm learning how to refactor code in a good way based on my experience and the experience of others. Today I would like to share an example of refactoring with you.

I got a code which was written by a person on internship long time ago in my company. The plan was to improve this code. It was mostly one class which you can see  <a href="https://github.com/womanonrails/poker/blob/55c9ae0ab921f7aa95bb7e47676d87b970a32033/lib/poker/hand.rb" title="Code before refactoring" target="_blank" rel="nofollow noopener noreferrer">here</a>. In this one class you have all rules to check hand in poker only for cards without jokers. Code is not bad. When you know business logic (in this case poker rules), you will be able to move in this code. This code has also tests. So, this is good news. It will be easier to change something if tests cover all logic.  If no, well we can break functionality, when we will be not careful. The code looks more procedural than object oriented and it has some duplications. Sometimes this will be enough. When this is written once and you will not need to change this code for long time **maybe** you don't need to clean it up. But if the requirements can/will change, probably your code too. You need to decide if it is better to refactor it now or later. (I prefer refactoring now, when I still remember logic and code, then later when I need to understand it one more time.) I would like to show you how I do refactoring base on this example.
...
Base on tests without going deep into logic, I made some improvements in the code. I removed some condition and simplify this code.
...
All code you can find <a href="https://github.com/womanonrails/poker/blob/148429e4591638aef38b5b7abaab5e0198d805c0/lib/poker/hand.rb" title="Code after 2nd step of refactoring" target="_blank" rel="nofollow noopener noreferrer">here</a>. This change in my opinion improves readability a little bit.

After that all tests passed.

# Step 3 - Understand logic and try to simplify

Now when code is for me more readable, I can start changing the logic to simplify it. I have tests, so each of my changes relies on tests. I took first method. I removed it (remove all content inside). And start from beginning. This is what I got.
...
2. Run the tests (some tests are failing), understand logic
...
I don't know if you noticed this, but everywhere we put as argument `array`. And we have class, but we don't use initializer for this class. The second thing is that in many places we use something like `array.each {|item| hash [item / 4] += 1}` let's move this also for initializer and use class state instead calculate this everywhere.
...
So code like `array. map {|item| item / 4}` tells us which figure from 2 to A is on card and code like this `array. map {|item| item % 4}` represents the color of a card (&spades;, &clubs;, &hearts;, &diams;).
...
Example of method before change:
...
Base on Reek metric I noticed a lot of duplication in code. So I decide to move this code to one method and use one more time state of the class instance. Whole step you can find <a href="https://github.com/womanonrails/poker/blob/74c05d7480e7857d1e99d604169f6eed46279758/lib/poker/hand.rb" title="Code after 5th step of refactoring" target="_blank" rel="nofollow noopener noreferrer">here</a>. And this is a quick overview.

We change initalizer:
...
Thanks for `@colors` instance variable I can change `flush?` method from:
